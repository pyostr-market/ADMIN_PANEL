...
// Меню для каждой кнопки
const CRM_MENU = [
  { label: 'Пользователи', icon: <UsersIcon />, path: '/users' },
  { label: 'Заявки', icon: <OrdersIcon />, path: '/orders' },
  { label: 'Чаты', icon: <ChatsIcon />, path: '/chats' },
];

const CATALOG_MENU = [
  { label: 'Товары', icon: <ProductsIcon />, path: '/products' },
  { label: 'Категории', icon: <CatalogIcon />, path: '/categories' },
  { label: 'Бренды', icon: <CatalogIcon />, path: '/manufacturers' },
  { label: 'Поставщики', icon: <CatalogIcon />, path: '/suppliers' },
  { label: 'Типы товаров', icon: <CatalogIcon />, path: '/product-types' },
  { label: 'Атрибуты', icon: <CatalogIcon />, path: '/attributes' },
];

const WAREHOUSE_MENU = [
  { label: 'Товары', icon: <ProductsIcon />, path: '/warehouse/products' },
  { label: 'Прайсы', icon: <PriceIcon />, path: '/warehouse/prices' },
  { label: 'Настройки', icon: <SettingsIcon />, path: '/warehouse/settings' },
];
...
export function BottomNav({ currentPath }) {
  ...
  return (
    <>
      ...
      <nav className="bottom-nav">
        <ul className="bottom-nav__list">
          <li className="bottom-nav__item">
            ...
            >
              <CrmIcon />
              <span className="bottom-nav__label">CRM</span>
            </button>
          </li>
          <li className="bottom-nav__item">
            <button
          ...
          <li className="bottom-nav__item">
            ...
            >
              <WarehouseIcon />
              <span className="bottom-nav__label">Склад</span>
            </button>
          </li>
        </ul>
      </nav>
      ...
      {/* Мобильные сайдбары */}
      {['crm', 'catalog', 'warehouse'].map((menuName) => (
        <div
          key={menuName}
          className={`mobile-sidebar ${activeMenu === menuName ? 'mobile-sidebar--open' : ''}`}
        >
          <div className="mobile-sidebar__header">
            <h3 className="mobile-sidebar__title">
              {menuName === 'crm' && 'CRM'}
              {menuName === 'catalog' && 'Каталог'}
              {menuName === 'warehouse' && 'Склад'}
            </h3>
          ...
          <nav className="mobile-sidebar__nav">
            {getMenuItems(menuName).map((item, index) => (
              <a
                key={index}
                href={item.path}
                className={`mobile-sidebar__link ${isActivePath(item.path) ? 'mobile-sidebar__link--active' : ''}`}
              >
                {item.icon}
          ...
          <div className="mobile-sidebar__footer">
            <a href="/profile" className="mobile-sidebar__footer-btn">
              <ProfileIcon />
              <span>Профиль</span>
            </a>
            <button className="mobile-sidebar__footer-btn" onClick={toggleTheme}>
